...
As expected, when the word is in the final position in the sentence, the duration is longer.  

```{r duration-final}
# A simple boxplot of intensity
dur <- ggplot(pse_stress, aes(Position, Duration))
dur + geom_boxplot() + facet_wrap(~Final, ncol = 1) + coord_flip()
```

The table below demonstrates the mean duration for each of the six participants in both sentence final and sentence medial positions. 
...
```{r pitch}
# A simple boxplot of intensity
f0 <- ggplot(pse_stress, aes(Position, F0, fill=Final))
